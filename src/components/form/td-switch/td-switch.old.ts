...
export class TdSwitch extends TypeDiv implements ITdSwitch {
  ...
  override setup() {
    ...
    watch(
      switchDisabled,
      (disabled) => {
        console.warn('watch disabled , disabled is ', disabled);
        if (disabled) {
          console.log('switchDisabled is ', disabled);
          this.style.setObj($switchDisabledStyle);
          this.core.style.setObj($disabledStyle);
          this.left?.style.setObj($disabledStyle);
          this.right?.style.setObj($disabledStyle);
        } else {
          // this.style.remove('opacity');
          // this.core.style.setObj({ cursor: 'point' });
          // this.left?.style.setObj({ cursor: 'point' });
          // this.right?.style.setObj({ cursor: 'point' });
        }
      },
      { immediate: true }
    );
    ...
    const actualValue = computed(() => {
      return isControlled.get()
        ? (props.vModel?.get() as boolean | string | number)
        : false;
    });
    ...
    this.addEvents({
      click: (evt) => {
        evt?.preventDefault();
        console.warn(
          'switch click. this.modelValue is ',
    ...
    onMounted(() => {
      console.warn('TdSwitch onMounted ', checked.get());
      if (input.get()) {
        input.get()!.checked = checked.get();
      }
      // todo 直接watch switchDisabled 不起作用， opacity会设置为1 ？？？？
      if (switchDisabled.get()) {
        this.style.setObj($switchDisabledStyle);
      }
    });
