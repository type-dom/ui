...
import { computed, Signal, signal, toRef, unref } from '@type-dom/signals';
import { useNamespace } from '../../../../hooks/use-namespace';
import { TdTooltip } from '../../../feedback/td-tooltip/td-tooltip.class';
import useMenu from '../use-menu';
import { MenuItemRegistered, MenuProvider, SubMenuProvider } from '../types';
import { ITdMenuItem, TdMenuItemProps } from './td-menu-item.interface';
import { menuItemEmits } from './td-menu-item.const';
import './style/index';
...
export class TdMenuItem extends TypeFragment implements ITdMenuItem {
  ...
  override setup() {
    ...
    const { parentMenu, indexPath } = useMenu(instance, toRef(props.mIndex!));
    ...
    const active = computed(() => props.mIndex === rootMenu.activeIndex);
    const item = signal({
      index: props.mIndex,
      indexPath: indexPath.get(),
      active: active.get(),
    }) as Signal<MenuItemRegistered>;
