...
export class ImageLazyLoadExample extends TypeDiv {
  ...
  setup() {
    ...
    this.attr.addClass('demo-image__lazy');
    this.addChild(new For({
      data: urls,
      getter: (url: string) => new TdImage({
        src: url,
        lazy: true,
      })
    }))

    createClass('demo-image__lazy', {
      height: '400px',
      overflowY: 'auto'
    })
    createClass('demo-image__lazy td-image', {
      display: 'block',
      minHeight: '200px',
      marginBottom: '10px'
    });
    createClass('demo-image__lazy td-image:last-child', {
      marginBottom: '0'
    });
  }
}
